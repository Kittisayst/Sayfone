...
public class ReportPayLateFoodController implements JoMVC, ActionListener, ItemListener, MouseListener {

    private final ReportPayLateFoodView view;
    ...
    public ReportPayLateFoodController(ReportPayLateFoodView view, FinancialModel model) {
        this.view = view;
        this.model = model;
    }
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (event.isEvent(view.getBtn_back())) {
            ...
            for (FinancialModel student : listFinancials) {
                String months = financialService.getDebtFoodMonth(registerID, student.getStudentID());
                ...
                student.setFoodMonth(findMissingMonth);
            }
            if (view.getMonth() == 0) {
                ...
                for (FinancialModel student : listFinancials) {
                    if (student.getFoodMonth().equals("ຈ່າຍຄົບຖ້ວນ")) {

                    } else {
                        List<Integer> listMonths = mc.StringToArray(student.getFoodMonth());
                        listMonths.removeIf(month -> month > view.getMonth());
                        if (!listMonths.isEmpty()) {
                            student.setFoodMonth(listMonths.toString());
    ...
    private void ExportData() {
        loading.setTitle("ກຳລັງສ້າງ Excel ການຄ້າງຄ່າອາຫານຫ້ອງ: " + view.getClassName());
